...
namespace seal
{
        ...
        : pool_(move(pool)), sec_level_(sec_level)
    {
        ...
        if (parameters_set())
        {
            ...
            std::vector<uint64_t, aligned_allocator<uint64_t>> root_of_unity_powers(root_of_unity_powers_size);

            keyswitch::loadTwiddleFactors(
                coeff_count, key_modulus.size(), key_modulus.data(), root_of_unity_powers.data());

            keyswitch::buildModulusMeta(key_modulus.size(), key_modulus.data(), modswitch_factors, &modulus_meta_);

            keyswitch::buildInvnMeta(
                coeff_count, key_modulus.size(), key_modulus.data(), root_of_unity_powers.data(), &invn_);

            const char *binfile =
                (nullptr == (binfile = getenv("XCLBIN_PATH"))) ? "/usr/local/share/keyswitch.xclbin" : binfile;
            auto fileBuf = xcl::read_binary_file(binfile);
            ...
            for (auto &dev : devices)
            {
                ...
                OCL_CHECK(err, context_ = cl::Context(dev, nullptr, nullptr, nullptr, &err));
                program = cl::Program(context_, { dev }, bins, nullptr, &err);
                if (err == CL_SUCCESS)
                {
                    device = dev;
            ...
            OCL_CHECK(err, broadcast_keys_krnl_ = cl::Kernel(program, "broadcast_keys", &err));
            OCL_CHECK(err, intt1_push_krnl_ = cl::Kernel(program, "intt1_push", &err));
            OCL_CHECK(err, intt1_twiddles_krnl_ = cl::Kernel(program, "dispatch_intt1_twiddles", &err));
            OCL_CHECK(err, intt2_twiddles_krnl_ = cl::Kernel(program, "dispatch_intt2_twiddles", &err));
            OCL_CHECK(err, mod_invn_krnl_ = cl::Kernel(program, "dispatch_mod_invn", &err));
            OCL_CHECK(err, ntt1_twiddles_krnl_ = cl::Kernel(program, "dispatch_ntt1_twiddles", &err));
            OCL_CHECK(err, ntt2_twiddles_krnl_ = cl::Kernel(program, "dispatch_ntt2_twiddles", &err));
            OCL_CHECK(err, store_krnl_ = cl::Kernel(program, "store", &err));

            OCL_CHECK(err, in_cq_ = cl::CommandQueue(context_, device, 0, &err));
            OCL_CHECK(err, out_cq_ = cl::CommandQueue(context_, device, 0, &err));
            OCL_CHECK(err, cq_ = cl::CommandQueue(context_, device, CL_QUEUE_OUT_OF_ORDER_EXEC_MODE_ENABLE, &err));

            cl_mem_ext_ptr_t intt1_tw_ext = { (unsigned)HBM(2), root_of_unity_powers.data(), nullptr };
            cl_mem_ext_ptr_t intt2_tw_ext = { (unsigned)HBM(1), root_of_unity_powers.data() + 6 * coeff_count,
                                              nullptr };
            cl_mem_ext_ptr_t ntt_tw_ext = { (unsigned)HBM(3), root_of_unity_powers.data() + 7 * coeff_count, nullptr };

            OCL_CHECK(
                err, intt1_twiddles_ = cl::Buffer(
                         context_, CL_MEM_WRITE_ONLY | CL_MEM_EXT_PTR_XILINX | CL_MEM_USE_HOST_PTR,
                         6 * coeff_count * sizeof(uint64_t), &intt1_tw_ext, &err));
            OCL_CHECK(
                err, intt2_twiddles_ = cl::Buffer(
                         context_, CL_MEM_WRITE_ONLY | CL_MEM_EXT_PTR_XILINX | CL_MEM_USE_HOST_PTR,
                         coeff_count * sizeof(uint64_t), &intt2_tw_ext, &err));
            OCL_CHECK(
                err, ntt_twiddles_ = cl::Buffer(
                         context_, CL_MEM_WRITE_ONLY | CL_MEM_EXT_PTR_XILINX | CL_MEM_USE_HOST_PTR,
                         7 * coeff_count * sizeof(uint64_t), &ntt_tw_ext, &err));

            OCL_CHECK(err, err = cq_.enqueueMigrateMemObjects({ intt1_twiddles_, intt2_twiddles_, ntt_twiddles_ }, 0));
            cq_.finish();
        }
#endif
    }
