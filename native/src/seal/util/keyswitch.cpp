    ...
    namespace util {
        namespace keyswitch {
            ...
            void loadTwiddleFactors(uint64_t n, uint64_t key_modulus_size,
                    const Modulus *moduli, uint64_t *root_of_unity_powers_ptr)
            {
                for (uint64_t i = 0; i < key_modulus_size; i++) {
                    ComputeRootOfUnityPowers(
                        moduli[i].value(), n, Log2(n),
                        MinimalPrimitiveRoot(2 * n, moduli[i].value()),
            ...
            void buildModulusMeta(uint64_t key_modulus_size, const Modulus *moduli,
                    const MultiplyUIntModOperand* modswitch_factors, moduli_t *modulus_meta)
            {
                for (uint64_t i = 0; i < key_modulus_size; i++) {
                    (*modulus_meta).data[i][0] = moduli[i].value();
                    (*modulus_meta).data[i][1] =
                        MultiplyFactor(1, 64, moduli[i].value()).BarrettFactor();
            ...
            void buildInvnMeta(uint64_t n, uint64_t key_modulus_size, const Modulus *moduli,
                    const uint64_t* root_of_unity_powers_ptr, moduli_t *invn)
            {
                for (uint64_t i = 0; i < key_modulus_size; i++) {
                    ...
                    uint64_t y_barrett_nw =
                        DivideUInt128UInt64Lo(inv_nw, 0, moduli[i].value());
                    (*invn).data[i][0] = inv_n;
            ...
            void readOutput(uint64_t n, uint64_t decomp_modulus_size,
                    const Modulus *moduli, const uint64_t *fpga_result,
